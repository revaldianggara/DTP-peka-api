...
angular.module("layerControl").controller("dashboardCtrl", ["$scope", "$http", "$mdSidenav", "$timeout", "mapService_dashboard", "$mdDialog", "$mdToast", "$interval", dashboardCtrl]);

function dashboardCtrl($scope, $http, $mdSidenav, $timeout, mapService_dashboard, $mdDialog, $mdToast, $interval) {
  const ctime = new Date();
  const webdomain = "http://karhutla.ai-innovation.id/dashboard/user/";
  $scope.toggle = {};
  var falarm1 = false;
  var falarm2 = false;
  var falarm3 = false;
  ...
  $scope.phsOnYear = function () {
    ...
    $http({
      method: "GET",
      url: "/user/webgis/selectYearPredHS_v2",
      params: {
        year: ynow,
      },
    }).then(
    ...
    $scope.predDateGroup.filter(function (hsit) {
      if (hsit.tid == tid_chk) {
        if (hsit.check) {
          ...
          $http({
            ...
            function querySuccess(response) {
              if (response.data.features && response.data.features.length > 0) {
                $scope.total_predHS = mapService_dashboard.drawHS("predHS_" + tid_chk, response.data);
          ...
          mapService_dashboard.removeHS("predHS_" + tid_chk);
        }
      }
    });
  };
    ...
    $scope.hsDateGroup.filter(function (hsit) {
      if (hsit.tid == tid_chk) {
        if (hsit.check) {
          ...
          $http({
            ...
            function querySuccess(response) {
              if (response.data.features.length > 0) {
                mapService_dashboard.drawHS("HS_" + tid_chk, response.data);
              }
            },
            function queryError(response) {
              console.log("connection failed");
          ...
          console.log("close " + tid_chk);
          mapService_dashboard.removeHS("HS_" + tid_chk);
        }
      }
    });
  };
    ...
    $scope.vegDateGroup.filter(function (hsit) {
      if (hsit.tid == tid_chk) {
        if (hsit.check) {
          ...
          $http({
            ...
            function querySuccess(response) {
              if (response.data.features.length > 0) {
                ...
                mapService_dashboard.drawHS("Veg_" + tid_chk, response.data);
              }
            },
            function queryError(response) {
              console.log("connection failed");
          ...
          console.log("close " + tid_chk);
          mapService_dashboard.removeHS("Veg_" + tid_chk);
        }
      }
    });
  };
    ...
    $scope.modisDateGroup.filter(function (hsit) {
      if (hsit.ur == dur) {
        if (hsit.check) {
          ...
          console.log(xurl);
          mapService_dashboard.addRasterLayer(xurl, inm, 10);
        } else {
          console.log("id to remove " + inm);
          mapService_dashboard.removeRasterLayer(inm);
        }
      }
    });
  };
