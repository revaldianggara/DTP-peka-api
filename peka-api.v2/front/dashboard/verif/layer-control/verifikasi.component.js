...
angular.module("layerControl").controller("verifCtrl", ["$scope", "$http", "$mdSidenav", "$timeout", "mapService_verif", "$mdDialog", "$mdToast", verifCtrl]);

function verifCtrl($scope, $http, $mdSidenav, $timeout, mapService_verif, $mdDialog, $mdToast) {
  // webgis
  ...
  const webdomain = "http://karhutla.ai-innovation.id/dashboard/verif/";
  $scope.toggle = {};
  var falarm1 = false;
  var falarm2 = false;
  var falarm3 = false;
  ...
  $scope.miscDateGroup = [];
  $scope.hs = mapService_verif.info_detail;
  ...
  $scope.addAction = function () {
    ...
    $mdToast
      ...
      .then(function (result) {
        if (result === "finish") {
          const tmp = mapService_verif.getGeoJson();
          ...
          mapService_verif.interactionFinished();
          $mdDialog
            .show({
              scope: $scope,
              preserveScope: true,
  ...
  $scope.addrec = function () {
    $http({
      ...
      params: {
        coord: $scope.penanganan.coord,
        info: $scope.penanganan.info,
        action: $scope.penanganan.action,
      },
    }).then(
    ...
    $scope.recDateGroup.filter(function (hsit) {
      if (hsit.id == tid_chk) {
        if (hsit.check) {
          ...
          $http({
            ...
            function querySuccess(response) {
              if (response.data.features.length > 0) {
                mapService_verif.drawHS("rec_" + tid_chk, response.data);
              }
            },
            function queryError(response) {
              console.log("connection failed");
          ...
          mapService_verif.removeHS("rec_" + tid_chk);
        }
      }
    });
  };
      ...
      if (hsit.tid == tid_chk) {
        if (hsit.check) {
          ...
          $http({
            ...
            function querySuccess(response) {
              if (response.data.features.length > 0) {
                mapService_verif.drawHS("predHS_" + tid_chk, response.data);
              }
            },
            function queryError(response) {
              console.log("connection failed");
          ...
          console.log("close " + tid_chk);
          mapService_verif.removeHS("predHS_" + tid_chk);
        }
      }
    });
  };
      ...
      if (hsit.tid == tid_chk) {
        if (hsit.check) {
          ...
          $http({
            ...
            function querySuccess(response) {
              if (response.data.features.length > 0) {
                mapService_verif.drawHS("HS_" + tid_chk, response.data);
              }
            },
            function queryError(response) {
              console.log("connection failed");
          ...
          console.log("close " + tid_chk);
          mapService_verif.removeHS("HS_" + tid_chk);
        }
      }
    });
  };
    ...
    $scope.vegDateGroup.filter(function (hsit) {
      if (hsit.tid == tid_chk) {
        if (hsit.check) {
          ...
          $http({
            ...
            function querySuccess(response) {
              if (response.data.features.length > 0) {
                ...
                mapService_verif.drawHS("Veg_" + tid_chk, response.data);
              }
            },
            function queryError(response) {
              console.log("connection failed");
          ...
          console.log("close " + tid_chk);
          mapService_verif.removeHS("Veg_" + tid_chk);
        }
      }
    });
  };
    ...
    $scope.modisDateGroup.filter(function (hsit) {
      if (hsit.ur == dur) {
        if (hsit.check) {
          ...
          console.log(xurl);
          mapService_verif.addRasterLayer(xurl, inm, 10);
        } else {
          console.log("id to remove " + inm);
          mapService_verif.removeRasterLayer(inm);
        }
      }
    });
  };
