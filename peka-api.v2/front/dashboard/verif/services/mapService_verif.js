angular
  .module("mapService_verif", [])
  .factory("mapService_verif", function ($mdSidenav) {
    var mapObj;
    var viewObj;
    var draw;
    var pointsource;
  ...
  .directive("ngMap", [
    "mapService_verif",
    "$http",
    function (mapService_verif, $http) {
      return {
        restrict: "A",
        replace: true,
        link: function ($scope) {
          ...
          map_verif.addLayer(IndonesiaPolygon);
          map_verif.addLayer(vectorIDN);
          vectorIDN.setZIndex(30);
          map_verif.on("click", function (e) {
            var wclk = 0;
            map_verif.forEachFeatureAtPixel(e.pixel, function (feature, layer) {
              const lyrnm = layer.get("name");
              ...
              if (lyrty == "predHS" || lyrty == "HS" || lyrty == "Veg" || lyrty == "rec") {
                $http({
                  ...
                  function querySuccess(response) {
                    ...
                    angular.copy(response.data, mapService_verif.info_detail);
                    mapService_verif.selectHS(feature, lyrnm);
                  },
            ...
            if (wclk == 0) {
              mapService_verif.selectHS(undefined, undefined);
              mapService_verif.hs_clicked = undefined;
              mapService_verif.type_clicked = undefined;
            }
          });
          $("#map_verif").data("map_verif", map_verif);
          mapService_verif.mapInit(map_verif, view);
        },
      };
    },
  ]);