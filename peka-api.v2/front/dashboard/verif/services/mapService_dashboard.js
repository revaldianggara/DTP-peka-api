angular.module("mapService_dashboard", [])
  // $mdSidenav
  .factory('mapService_dashboard', function ($mdSidenav) {
    var mapObj;
    var viewObj;
    var draw;
    var pointsource;
  ...
  .directive('ngMap', ['mapService_dashboard', '$http', function (mapService_dashboard, $http) {
    return {
      restrict: 'A',
      replace: true,
      link: function ($scope) {
        ...
        map.on("click", function (e) {
          ...
          map.forEachFeatureAtPixel(e.pixel, function (feature, layer) {
            ...
            if (lyrty == 'predHS' || lyrty == 'HS' || lyrty == 'Veg' || lyrty == 'rec') {
              $http({
                ...
                angular.copy(response.data, mapService_dashboard.info_detail);
                mapService_dashboard.selectHS(feature, lyrnm);
          ...
          if (wclk == 0) {
            mapService_dashboard.selectHS(undefined, undefined);
            mapService_dashboard.hs_clicked = undefined;
            mapService_dashboard.type_clicked = undefined;
          }
        });
        $('#map').data('map', map);
        mapService_dashboard.mapInit(map, view);
      }
    };
  }]);