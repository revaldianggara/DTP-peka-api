...
angular.module("addnewinput").controller("addnewinputdialog_ph", ["$scope", "$mdDialog", "$http", "FileUploader", addnewinputdialog_ph]);

function addnewinputdialog_ph($scope, $mdDialog, $http, FileUploader) {
  $scope.dyear = [];
  $scope.dperiod = [];
  $scope.inputfeature = [];
  $scope.progressper = 0;
  ...
  $scope.loadFeature = function () {
    return $http({
      method: "GET",
      url: "/user/input/PotensiHotspot/getFeatureType",
    }).then(
  ...
  $scope.checkPeriod = function () {
    return $http({
      method: "GET",
      url: "/user/input/PotensiHotspot/getCheckPeriod",
      params: {
  ...
  $scope.addNewInputFeatureName = function (ev) {
    ...
    $mdDialog.show(confirm).then(
      function (result) {
        ...
        $http({
          method: "GET",
          url: "/user/input/PotensiHotspot/getNewFeature",
          params: {
            nof: result,
          },
        }).then(
  ...
  var uploader = ($scope.uploader = new FileUploader({
    url: "input/PotensiHotspot/getUploadFeature",
    method: "POST",
  }));
