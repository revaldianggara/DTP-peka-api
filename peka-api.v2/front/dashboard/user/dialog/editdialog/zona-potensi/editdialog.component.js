...
angular.
module('editdialog').
controller('editdialog_zp', ['$scope', '$mdDialog', '$http', 'dataToPass', editmodeldialog_zp]);

function editmodeldialog_zp($scope, $mdDialog, $http, dataToPass) {
    var mlid = dataToPass;
    $scope.newModel = {};
    $scope.mltype = {};
    ...
    $scope.loadModelConf = function () {
        $http({
            method: "GET",
            url: "/user/model_zp/getModelConf",
            params: {
                modid: mlid
            }
    ...
    $scope.loadInputConf = function () {
        $http({
            method: "GET",
            url: "/user/model_zp/getModelInput",
            params: {
                modid: mlid
            }
        }).then(function querySuccess(response) {
    ...
    $scope.mlConfig = function (ev) {
        $mdDialog.show({
                scope: $scope,
                templateUrl: 'dialog/editdialog/zona-potensi/mltemplate/' + $scope.newModel.mltype + '.template.html',
