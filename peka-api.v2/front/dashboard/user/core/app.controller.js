(function () {
    ...
    function AppCtrl($scope, $rootScope, $route, $document) {
        ...
        $scope.$watch('scientist', function (newVal, oldVal) {
            if (newVal.menu === 'horizontal' && oldVal.menu === 'vertical') {
                $rootScope.$broadcast('nav:reset');
            }
            if (newVal.fixedHeader === false && newVal.fixedSidebar === true) {
                if (oldVal.fixedHeader === false && oldVal.fixedSidebar === false) {
                    $scope.scientist.fixedHeader = true;
                    $scope.scientist.fixedSidebar = true;
                }
                if (oldVal.fixedHeader === true && oldVal.fixedSidebar === true) {
                    $scope.scientist.fixedHeader = false;
                    $scope.scientist.fixedSidebar = false;
                }
            }
