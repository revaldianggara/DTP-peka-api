...
angular.
module('addnewmodel').
controller('addnewmodeldialog_ph', ['$scope', '$mdDialog', '$timeout', '$http', addnewmodeldialog_ph]);

function addnewmodeldialog_ph($scope, $mdDialog, $timeout, $http) {
        var inptp = [];
        var inpyr = [];
        var model_slc;
        $scope.newModel = {};
        ...
        $scope.getInputType = function () {
                $http({
                        method: "GET",
                        url: "/user/model_ph/getFeatureType"
        ...
        $scope.getYearComb = function (idret, idc) {
                $http({
                        method: "GET",
                        url: "/user/model_ph/getFeatureCombineYear",
                        params: {
                                inpid: idret,
                                countid: idc
                        }
        ...
        $scope.mlConfig = function (ev) {
                ...
                $mdDialog.show({
                                scope: $scope,
                                templateUrl: 'dialog/addnewmodel/potensi-hotspot/mltemplate/' + model_slc + '.template.html',
        ...
        $scope.addNewModel = function () {
                $scope,
                newModel.mlprop = JSON.stringify($scope.mltyle);
                console.log($scope.newModel);
                $http({
                        method: "GET",
                        url: "/user/model_ph/getAddModel",
                        params: {
                                nm: $scope.newModel
                        }
                }).then(function querySuccess(response) {
