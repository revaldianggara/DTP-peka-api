angular.module("mapService_satelit", [])
.factory('mapService_satelit', function($mdSidenav) {
    var mapObj;
    var viewObj;
    var draw;
    var pointsource;
...
.directive('ngMap', ['mapService_satelit', '$http', function (mapService_satelit, $http) {
    return {
        restrict: 'A',
        replace: true,
        link: function ($scope) {
            ...
            map.on("click", function(e) {
                ...
                map.forEachFeatureAtPixel(e.pixel, function (feature, layer) {
                    ...
                    if (lyrty == 'predHS' || lyrty == 'predVeg' || lyrty == 'rec') {
                        $http({
                            ...
                            angular.copy(temp, mapService_satelit.info_detail);
                            mapService_satelit.selectHS(feature, lyrnm);
                ...
                if (wclk == 0) {
                    mapService_satelit.selectHS(undefined, undefined);
                    mapService_satelit.hs_clicked = undefined;
                    mapService_satelit.type_clicked = undefined;
                }
            });
            $('#map').data('map', map);
            mapService_satelit.mapInit(map, view);
        }
    };
}]);
