...
angular.
    module('addnewinput').
    controller('addnewinputdialog', ['$scope', '$mdDialog', '$http', 'FileUploader', addnewinputdialog]);
        function addnewinputdialog($scope, $mdDialog, $http, FileUploader) {
            $scope.dyear = [];
            $scope.dperiod = [];
            $scope.inputfeature = [];
            $scope.progressper = 0;
            ...
            $scope.checkPeriod = function() {
                return $http({
                    ...
                    params : {year : $scope.inputfeature.year, inname : $scope.inputfeature.name}
                }).then(function querySuccess(response) {
